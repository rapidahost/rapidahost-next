...
// 📤 Export Filtered Email Logs to CSV
...
{servicesTab === 'emails' && (
  <div className="overflow-x-auto space-y-3">
      ...
      <tbody>
        {paginatedEmails.map((email: any, i: number) => (
          <tr key={i} className="border-t">
            ...
            <td className="p-2 border">{email.status === 'Retried' ? '🔁 Retried' : email.status}</td>
    ...
    <div className="flex justify-between text-sm mt-2">
      <button disabled={emailPage === 1} onClick={() => setEmailPage(emailPage - 1)} className="px-3 py-1 rounded border bg-white disabled:opacity-50">← Prev</button>
      <div>Page {emailPage} of {Math.ceil(emailLogs.length / itemsPerPage)}</div>
      <button disabled={emailPage * itemsPerPage >= emailLogs.length} onClick={() => setEmailPage(emailPage + 1)} className="px-3 py-1 rounded border bg-white disabled:opacity-50">Next →</button>
    </div>
  </div>
)}
        ...
        {paginatedEmails.map((email: any, i: number) => (
          <tr key={i} className="border-t">
            ...
            <td className="p-2 border">
              {email.status}
              {email.status === 'retry' && <span className="ml-1">🔁</span>}
            </td>
            <td className="p-2 border">{new Date(email.timestamp).toLocaleString()}</td>
            <td className="p-2 border space-x-2">
              <Link href={`/admin/email/${email.messageId}`} className="text-blue-600 underline">View</Link>
...
// ... (โค้ดเดิมทั้งหมดด้านบนเหมือนเดิม)
const [servicesTab, setServicesTab] = useState<'services' | 'emails' | 'invoices'>('services');
const [invoicePage, setInvoicePage] = useState(1);
const invoicesPerPage = 5;

const clientInvoices = client.invoices || [];
const paginatedInvoices = clientInvoices.slice((invoicePage - 1) * invoicesPerPage, invoicePage * invoicesPerPage);

{servicesTab === 'invoices' && (
  <div className="overflow-x-auto space-y-3">
    <div className="text-sm font-medium">Total: {clientInvoices.length} invoices</div>
    <table className="w-full text-sm border">
      <thead>
        <tr className="bg-gray-100 text-left">
      ...
      <tbody>
        {paginatedInvoices.map((invoice: any, i: number) => (
          <tr key={i} className="border-t">
            <td className="p-2 border">{invoice.id}</td>
            <td className="p-2 border">{invoice.amount}</td>
            <td className="p-2 border">{invoice.status}</td>
            <td className="p-2 border">{new Date(invoice.date).toLocaleString()}</td>
            <td className="p-2 border space-x-2">
              <Link href={`/admin/invoice/${invoice.id}`} className="text-blue-600 underline">View</Link>
              <button onClick={() => resendInvoiceEmail(invoice.id)} className="text-orange-600 underline">Resend</button>
            </td>
          </tr>
        ))}
      </tbody>
    ...
    <div className="flex justify-between text-sm mt-2">
      <button disabled={invoicePage === 1} onClick={() => setInvoicePage(invoicePage - 1)} className="px-3 py-1 rounded border bg-white disabled:opacity-50">← Prev</button>
      <div>Page {invoicePage} of {Math.ceil(clientInvoices.length / invoicesPerPage)}</div>
      <button disabled={invoicePage * invoicesPerPage >= clientInvoices.length} onClick={() => setInvoicePage(invoicePage + 1)} className="px-3 py-1 rounded border bg-white disabled:opacity-50">Next →</button>
    </div>
  </div>
)}
