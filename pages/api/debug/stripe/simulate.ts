// pages/api/debug/stripe/simulate.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // 1) หา client เดิมจาก WHMCS ก่อน
    ...
    if (existing?.result === 'success' && (existing as any).clients?.client?.length) {
      ...
      // 2) ถ้าไม่เจอให้สร้าง client ใหม่
      ...
      const raw = (created as any).clientid as string | number
      clientId = typeof raw === 'string' ? parseInt(raw, 10) : Number(raw)
    }
